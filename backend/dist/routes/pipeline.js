...
async function pipelineRoutes(server) {
    server.get('/run-pipeline', async (request, reply) => {
        ...
        (0, child_process_1.exec)('npm run scrape && npm run process-articles', {
    ...
    server.get('/pipeline-health', async () => {
        const articles = await prisma_1.prisma.article.count();
        const ready = await prisma_1.prisma.script.count({ where: { status: 'ready' } });
        const completed = await prisma_1.prisma.script.count({ where: { status: 'completed' } });
    ...
    server.get('/scrape', async (request, reply) => {
        ...
        (0, child_process_1.exec)('npm run scrape', {
            cwd: '/opt/render/project/src/backend',
            env: process.env
        }, (error, stdout, stderr) => {
            if (error)
                console.error('❌ Scrape failed:', error.message, stderr);
            else
