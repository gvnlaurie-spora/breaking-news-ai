...
async function uploadToYouTube(videoPath, title, description) {
    ...
    const response = await youtube.videos.insert({
        ...
        media: {
            mimeType: "video/mp4",
            body: (0, fs_1.createReadStream)(videoPath),
        },
    });
...
async function buildMetadataFromDB() {
...
async function main() {
    ...
    const videosDir = (0, path_1.resolve)(process.cwd(), "output", "videos");
    let files = [];
    try {
        files = (0, fs_1.readdirSync)(videosDir)
            .filter(f => f.endsWith(".mp4") && !f.includes("test") && !f.includes("dummy"))
            .sort((a, b) => {
            const aStat = (0, fs_1.statSync)((0, path_1.resolve)(videosDir, a));
            const bStat = (0, fs_1.statSync)((0, path_1.resolve)(videosDir, b));
    ...
    const videoPath = (0, path_1.resolve)(videosDir, latest);
    const fileSizeMB = ((0, fs_1.statSync)(videoPath).size / 1024 / 1024).toFixed(1);
