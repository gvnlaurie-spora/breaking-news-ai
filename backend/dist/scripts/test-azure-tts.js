...
async function testAzureTTS() {
    ...
    try {
        await (0, tts_1.generateTTS)("Hello! This is a test of the Azure Text to Speech service.", "/tmp/test-azure.mp3", "male");
        ...
        exec('ffplay /tmp/test-azure.mp3', (error) => {
            if (error)
                console.log("Play with: ffplay /tmp/test-azure.mp3");
        });
    }
    catch (error) {
