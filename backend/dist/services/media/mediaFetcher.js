...
exports.fetchPexelsImage = fetchPexelsImage;
exports.fetchPexelsVideo = fetchPexelsVideo;
exports.fetchPixabayVideo = fetchPixabayVideo;
exports.fetchArchiveVideo = fetchArchiveVideo;
exports.fetchPollinationsImage = fetchPollinationsImage;
exports.fetchMediaForStory = fetchMediaForStory;
...
const MEDIA_TMP = path_1.default.resolve(process.cwd(), 'output', 'media');
fs_1.default.mkdirSync(MEDIA_TMP, { recursive: true });
// ─────────────────────────────────────────────
// HELPERS
// ─────────────────────────────────────────────
async function downloadFile(url, destPath) {
    try {
        const response = await (0, axios_1.default)({ url, method: 'GET', responseType: 'stream', timeout: 30000 });
        await streamPipeline(response.data, fs_1.default.createWriteStream(destPath));
        return fs_1.default.existsSync(destPath) && fs_1.default.statSync(destPath).size > 10000;
    }
    catch {
        return false;
    }
...
// PEXELS IMAGES
// ─────────────────────────────────────────────
async function fetchPexelsImage(query, destPath) {
    if (!PEXELS_API_KEY)
        return false;
    try {
        const res = await axios_1.default.get('https://api.pexels.com/v1/search', {
...
// PEXELS VIDEO
// ─────────────────────────────────────────────
async function fetchPexelsVideo(query, destPath) {
    if (!PEXELS_API_KEY)
        return false;
    try {
...
// PIXABAY VIDEO
// ─────────────────────────────────────────────
async function fetchPixabayVideo(query, destPath) {
    if (!PIXABAY_API_KEY)
        return false;
    try {
        const res = await axios_1.default.get('https://pixabay.com/api/videos/', {
...
// ─────────────────────────────────────────────
async function fetchArchiveVideo(query, destPath) {
    try {
        ...
        for (const doc of docs) {
            try {
                const metaRes = await axios_1.default.get(`https://archive.org/metadata/${doc.identifier}`, {
                    timeout: 10000,
                });
...
async function fetchPollinationsImage(prompt, destPath) {
    try {
        const encoded = encodeURIComponent(`News photography style, photojournalism, 16:9, ${prompt}. No text overlays.`);
        ...
        const res = await (0, axios_1.default)({ url, method: 'GET', responseType: 'stream', timeout: 45000 });
        await streamPipeline(res.data, fs_1.default.createWriteStream(destPath));
        return fs_1.default.existsSync(destPath) && fs_1.default.statSync(destPath).size > 5000;
    }
    catch {
        return false;
    }
}
async function fetchMediaForStory(title, storyId) {
    const keywords = extractKeywords(title);
    const result = { images: [], videos: [] };
    console.log(`  🔍 Fetching media for: "${keywords}"`);
    // --- IMAGES ---
    // Try Pexels first, then Pollinations as fallback
    const img1 = path_1.default.join(MEDIA_TMP, `${storyId}_img1.jpg`);
    const img2 = path_1.default.join(MEDIA_TMP, `${storyId}_img2.jpg`);
    ...
    const vid1 = path_1.default.join(MEDIA_TMP, `${storyId}_vid1.mp4`);
    const vid2 = path_1.default.join(MEDIA_TMP, `${storyId}_vid2.mp4`);
